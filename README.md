...
- **桌面玩偶「伙伴」**：工作台右下角常驻的 2D 小工程师，随你的工作状态变化
...
### 桌面玩偶「伙伴」（v1.2.0，v1.2.1 精致化）

工作台右下角常驻一个 2D 小工程师玩偶（纯 SVG/CSS 绘制，不依赖任何图片资源），它会**跟随你的工作状态变化**：

| 状态 | 表现 |
|---|---|
| 专注中（番茄钟运行） | 戴上耳机埋头画图，耳机灯呼吸闪烁，面带屏幕光效 |
| 休息中（短/长休息） | 喝咖啡放松，咖啡杯冒着蒸汽 |
| 完成时（番茄阶段结束 / 待办勾选完成） | 蹦跳庆祝 + 张嘴大笑 + 手臂举起 + 彩色粒子彩带 |
| 空闲时 | 漂浮待机，随机眨眼 |
| 久坐时（空闲 60 秒） | 打盹冒 Zzz，戳一下唤醒 |

- 点击玩偶，它会随机回你一句工程师台词
- **按住玩偶可以拖拽到工作台任意位置**：拖拽时有"拎起"悬浮感，靠近屏幕边缘自动吸附贴边，窗口变化自动校正位置
- 「伙伴」标签页：**切换角色（男工程师 / 女工程师）**、给玩偶**改名字**、**调整大小**（60–150px）、**开关常驻显示**
- 渐变质感、表情系统（眉 + 多嘴型）、呼吸起伏动画，空闲时还会随机歪头卖萌；气泡为毛玻璃风格
- 深色 / 浅色主题下都能正常显示

女工程师「小E妹」（齐刘海 + 双马尾 + 蝴蝶结发饰）与五种状态：

![女工程师五种状态](images/screenshots/pet-female-moods.png)

男工程师「小EE」与五种状态：

![男工程师五种状态](images/screenshots/pet-moods.png)
