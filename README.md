...
- **番茄钟**：用于专注设计、调试或学习
- **build-your-own-x**：整理 DIY 项目、复刻灵感和学习笔记
- **待办清单**：管理当前任务、Bug、实验记录和后续优化项
...
`build-your-own-x` 模块用于收集和整理你想亲手实现或复刻的硬件项目。
...
这个模块适合硬件工程师、创客和嵌入式开发者用来沉淀长期项目灵感。
...
它适合把零散任务从脑子里转移到清单中，避免在复杂设计过程中遗漏细节。
...
- 如果插件需要联网功能，需要在扩展管理器中开启“外部交互”（当前版本无需联网）
...
```
build/dist/ee-workbench_v1.0.0.eext
```

### 本地调试

在嘉立创 EDA 专业版中打开扩展管理器，导入 `build/dist/ee-workbench_v1.0.0.eext`（或直接导入本工程目录），然后通过顶部菜单「EE Workbench」打开工作台。
